...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(51, 124, 176, it_anim->second.active_rect_alpha), 3.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    const ImS64 win_size_v = (ImMax(size_contents_v, size_avail_v), (ImS64)1);
    ...
    const ImS64 scroll_max = ((ImS64)1, size_contents_v - size_avail_v);
    ...
    window->DrawList->AddRectFilled(grab_rect.Min + ImVec2(5, 5), grab_rect.Max - ImVec2(5, 5), ImGui::GetColorU32(ImGuiCol_CheckMark), 3);
...
bool ImGui::Checkbox(const char* label, bool* v, bool offset)
{
    ...
    const ImRect check_bb(window->DC.CursorPos + ImVec2(15, 0), window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 2 + 15, label_size.y + style.FramePadding.y * 2));
    ItemSize(check_bb, style.FramePadding.y);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(15, style.FramePadding.y), window->DC.CursorPos + ImVec2(15, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
    ...
    it_filled->second = ImClamp(it_filled->second + (2.50f * ImGui::GetIO().DeltaTime * ((*v) ? 2.f : -2.f)), it_hover->second, 1.f);
    ...
    RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(27, 27, 27, (int)(255 * g.Style.Alpha)), false, 4);
    RenderFrame(check_bb.Min + ImVec2(pad + 1, pad + 1), check_bb.Max - ImVec2(pad + 1, pad + 1), ImColor(15, 15, 15, int(255 * GetStyle().Alpha)), false, 4);
    RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(19, 148, 250, (int)(255 * it_filled->second)), false, 4);
    if (*v)
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(7, 5), ImColor(255, 255, 255, (int)(255 * it_filled->second)), 9);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
 ...
 //   window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);
   ...
   // window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13), 4.f + it_anim->second.active_circle_anim, ImColor(51, 124, 176, 175));
...
ImVec4 ToVvec4(float r, float g, float b, float a)
{
    return ImVec4(r / MAX_RGB, g / MAX_RGB, b / MAX_RGB, a / MAX_RGB);
}

static ImVec4 active = ToVvec4(51, 124, 176, 175);
static ImVec4 inactive = ToVvec4(125, 125, 125, 85);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (it_anim->second.clicked)
        it_anim->second.circle_anim += 0.5f * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.circle_anim -= 0.2f * (1.f - ImGui::GetIO().DeltaTime);

    if (it_anim->second.circle_anim > 2.5)
        it_anim->second.circle_anim = 2.5;
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f),ImColor(51, 124, 176, 255));

    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_anim->second.circle_anim, it_anim->second.clicked ? ImColor(52, 142, 207, 255) : ImColor(51, 124, 176, 255), 30.f);
